...
Player Cap is a native BriefcaseNative server mod that changes the maximum number of players accepted by a Deceive Inc. dedicated server.
...
4. Keep an existing `Briefcase/Mods/briefcase.player-cap/Data/config.json` file when updating.
...
The configuration is stored in `Briefcase/Mods/briefcase.player-cap/Data/config.json`. It can also be edited through the Briefcase server administration interface.
